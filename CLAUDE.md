...
| ツール | 引数 | 動作 |
|--------|------|------|
| `list_knowledge` | scope?: str | 利用可能なナレッジファイルの一覧と概要を返す（本文は含まない） |
| `knowledge_search` | query: str | index.mdの内容から、質問に関連するナレッジを推薦する |
| `read_knowledge` | path: str | 指定ナレッジファイルの全文を返す（オンデマンドロード） |
| `keyword_search` | pattern: str, scope?: str | 全ナレッジファイルをgrep検索。正規表現対応。前後N行の文脈付きで返す |
...
セッション開始時にはナレッジの**説明のみ**をロードし、
本文はLLMが`read_knowledge`を呼んだ時に初めてロードする。
これによりコンテキストの無駄遣いを防ぐ。

```
セッション開始時にロードされるもの:
  - システムプロンプト（ツール定義含む）
  - knowledge/index.md（各ナレッジの名前と1行概要のみ）
  - project_memory.md（先頭25KB上限）

LLMが必要と判断した時にロードされるもの:
  - 個別のナレッジファイル本文（read_knowledge経由）
...
### F1: ナレッジ取り込み（knowledge_ingest）

任意形式のドキュメントをナレッジファイルに変換する事前処理。

| 項目 | 内容 |
|------|------|
| 入力 | 任意のファイル/ディレクトリ |
| 出力 | knowledge/{doc_name}/ 配下にトピック単位.md + index.md |
...
入力ファイル
  ...
  → [オプション] LLMで構造化
  → knowledge/{doc_name}/section.md として保存
  → index.md 生成（ナレッジ名 + 1行概要）
```
...
graph TD
    subgraph Prep[事前準備]
        ...
        MD --> SPLIT[章分割] --> STR[LLM構造化] --> SK[knowledge/*.md]
        SK --> IDX[index.md]
    end
...
├── tools/
│   ├── search.py              # list_knowledge, knowledge_search, read_knowledge, keyword_search
...
| 3 | オンデマンドロード | 概要のみ先行、本文はread_knowledge時 |
...
- **keyword_searchのインデックス**: ナレッジ数が少なければgrep、多ければ事前構築
