...
| 1-4 | `skills/index.md` | スキルインデックス初期ファイル | 0.5h |
...
| ツール | 実装内容 | 工数目安 |
|--------|---------|---------|
| `list_skills` | `skills/index.md` を読んで構造化して返す | 0.5h |
| `skill_search` | index.mdの内容をクエリでフィルタ・スコアリング | 1h |
| `read_skill` | 指定パスのスキルファイル全文読み込み + リマインダー付与 | 0.5h |
| `keyword_search` | `skills/`配下を再帰的にgrep（正規表現対応）、前後N行付き | 1h |
...
**目標:** 任意ドキュメントをスキルファイルに変換するCLIを実装する

### ファイル: `knowledge_to_skills.py`
...
入力ファイル/ディレクトリ
  ...
  → skills/{doc_name}/section_N.md として保存
  → skills/index.md を更新（スキル名 + 1行概要を追記）
```
...
- セッション開始時に `skills/index.md` + `project_memory.md`（先頭25KB）を読み込み
...
| # | シナリオ | 確認項目 |
|---|---------|---------|
| T1 | PDFドキュメント取り込み | skills/配下にファイル生成, index.md更新 |
| T2 | 単純な質問（1ターン） | list_skills → skill_search → read_skill → 回答 |
| T3 | 複数ターンの検索 | keyword_search → read_skill → さらにkeyword_search |
...
> **最小動作版の定義:** `list_skills` / `read_skill` / `keyword_search` / `write_file` / `todo_write` の5ツールのみ実装し、エージェントループを動かす。静的解析・リーダーはPhase 4-5で後追い。
...
| U3 | keyword_searchの事前インデックス構築 | スキル数が多い場合にgrep速度が問題になる |
